...
void setup() {
  ...
  REG_TCC0_PER = 120;      // Set the frequency of the PWM on TCC0 to 25kHz
  while(TCC0->SYNCBUSY.bit.PER);

  REG_TCC0_CCB0 = 120*0.2;       // TCC0 CCB0 - 50% duty cycle on D0
  while(TCC0->SYNCBUSY.bit.CCB0);

  REG_TCC0_CCB1 = 120*0.2;       // TCC0 CCB0 - 50% duty cycle on D1
  while(TCC0->SYNCBUSY.bit.CCB1);

  REG_TCC1_PER = 24;
  while(TCC1->SYNCBUSY.bit.PER);

  REG_TCC1_CCB0 = 24/2;       // TCC0 CCB0 - 50% duty cycle on D2
  while(TCC1->SYNCBUSY.bit.CCB0);

  // dead time insertion
  REG_TCC0_WEXCTRL |= TCC_WEXCTRL_DTHS(1) | TCC_WEXCTRL_DTIEN1 | TCC_WEXCTRL_DTLS(50) | TCC_WEXCTRL_DTIEN0;
