...
#define CLK_DIV 1
#define CLK_ID 4
int previousICGDuty = -1; // Track the previous duty cycle value for ICG
int previousSHDuty = -1; // Track the previous duty cycle value for SH
...
void setup() {
  ...
  REG_TCC0_PER = 317255 - 1;  // Set the frequency of the PWM on TCC0 to 151Hz, use command P to change
  while (TCC0->SYNCBUSY.bit.PER)
    ;

  REG_TCC0_CCB0 = 5064 - 1;  // TCC0 CCB0 - 50% duty cycle on D0, command D ICG signal duty cycle
  while (TCC0->SYNCBUSY.bit.CCB0)
    ;

  REG_TCC0_CCB1 = 4800 - 1;  // TCC0 CCB0 - 50% duty cycle on D1, command C for duty cycle of sh
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (command == 'P') {  // Change period of ICG
      int newPeriod = Serial.parseInt();
      if (newPeriod > 0) {
        REG_TCC0_PER = newPeriod - 1;
        while (TCC0->SYNCBUSY.bit.PER)
          ;
        Serial.print("ICG period changed to ");
        Serial.println(newPeriod);
      } else {
        Serial.println("Invalid period value.");
      }
    } else if (command == 'D') {  // Change duty cycle of ICG
      int newDutyCycle = Serial.parseInt();
      if (newDutyCycle >= 0 && newDutyCycle <= REG_TCC0_PER && newDutyCycle != previousICGDuty) {
        REG_TCC0_CCB0 = newDutyCycle - 1;
        while (TCC0->SYNCBUSY.bit.CCB0)
          ;
        previousICGDuty = newDutyCycle;
        Serial.print("ICG duty cycle changed to ");
        Serial.println(newDutyCycle);
      } else if (newDutyCycle == previousICGDuty) {
        Serial.print("ICG duty cycle is already set to ");
        Serial.println(newDutyCycle);
      } else {
        Serial.println("Invalid duty cycle value.");
      }
    } else if (command == 'S') {  // Change period of SH
      int newPeriod = Serial.parseInt();
      if (newPeriod > 0) {
        REG_TCC0_PER = newPeriod - 1;
        while (TCC0->SYNCBUSY.bit.PER)
          ;
        Serial.print("SH period changed to ");
        Serial.println(newPeriod);
      } else {
        Serial.println("Invalid period value.");
      }
    } else if (command == 'C') {  // Change duty cycle of SH
      int newDutyCycle = Serial.parseInt();
      if (newDutyCycle >= 0 && newDutyCycle <= REG_TCC0_PER && newDutyCycle != previousSHDuty) {
        REG_TCC0_CCB1 = newDutyCycle - 1;
        while (TCC0->SYNCBUSY.bit.CCB1)
          ;
        previousSHDuty = newDutyCycle;
        Serial.print("SH duty cycle changed to ");
        Serial.println(newDutyCycle);
      } else if (newDutyCycle == previousSHDuty) {
        Serial.print("SH duty cycle is already set to ");
        Serial.println(newDutyCycle);
      } else {
        Serial.println("Invalid duty cycle value.");
      }
